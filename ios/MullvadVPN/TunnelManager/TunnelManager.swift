...
final class TunnelManager {
    ...
    func getNextKeyRotationDate() -> Date? {
        ...
        if case .some(let (lastAttemptDate, completion)) = lastKeyRotationData {
            if case .invalidDeviceState = completion.error {
                return nil
            }

            if case .deviceRevoked = completion.error {
                return nil
            }

            // Do not rotate the key if account or device is not found.
            if case .restError(.unhandledResponse(_, let serverErrorResponse)) = completion.error,
    ...
    private func didReconnectTunnel(completion: OperationCompletion<(), TunnelManager.Error>) {
        nslock.lock()
        defer { nslock.unlock() }

        if let error = completion.error {
            logger.error(chainedError: error, message: "Failed to reconnect the tunnel.")
        }
...
private struct TunnelInteractorProxy: TunnelInteractor {
    ...
    fileprivate init(_ tunnelManager: TunnelManager) {
